...
# Init the bounds
min_pop, max_pop = float('inf'), 0

# For evrey type of termite
for termite in rules:
    
    # Crate the population
    population = dict()
    ...
    final_population = get_population(population)
    
    # Update bounds if necesarry
    if final_population < min_pop:
        min_pop = final_population
    elif final_population > max_pop:
        max_pop = final_population
    
# Compute the results
result = max_pop - min_pop    

print(f'{result} is the difference between the highest and lowest final population')