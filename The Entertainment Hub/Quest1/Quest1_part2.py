...
for token in tokens:
    ...
    while start_x <= max_x:
        ...
        max_token_coins = max(get_coins(drops, token, start_x, max_y), max_token_coins)
        
        start_x += 2
        
    total_coins += max_token_coins
        
# Print the results
print(f'The total number of coins that can be won is {total_coins}')
